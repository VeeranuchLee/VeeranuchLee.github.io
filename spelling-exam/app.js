...
/* Mascot: hand-drawn inline SVG (princess-crown unicorn), so no asset file ships for it. */
const MASCOT=`<svg viewBox="0 0 320 240" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="horn" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ffe08a"/><stop offset="1" stop-color="#f6ad3d"/></linearGradient></defs><path d="M226,84 Q292,116 268,182 Q256,212 222,216 Q246,180 236,152 Q228,120 210,108 Z" fill="#c4b5fd"/><path d="M228,96 Q282,124 260,176 Q250,198 228,204 Q246,174 238,148 Q232,120 216,112 Z" fill="#e879f9"/><path d="M94,84 Q48,112 62,158 Q70,180 96,186 Q80,146 102,114 Z" fill="#f9a8d4"/><path d="M100,66 Q86,12 118,32 Q130,48 120,70 Z" fill="#fffdfb"/><path d="M104,60 Q96,28 116,40 Q122,52 116,64 Z" fill="#ffc9de"/><path d="M220,66 Q234,12 202,32 Q190,48 200,70 Z" fill="#fffdfb"/><path d="M216,60 Q224,28 204,40 Q198,52 204,64 Z" fill="#ffc9de"/><polygon points="160,16 145,80 175,80" fill="url(#horn)"/><path d="M150,62 Q160,69 170,62" stroke="#dba53d" stroke-width="5" fill="none" stroke-linecap="round"/><path d="M153,44 Q160,50 167,44" stroke="#dba53d" stroke-width="5" fill="none" stroke-linecap="round"/><ellipse cx="160" cy="138" rx="80" ry="72" fill="#fffdfb"/><ellipse cx="160" cy="176" rx="46" ry="28" fill="#ffedf4"/><path d="M128,86 Q140,58 162,64 Q186,58 194,86 Q178,72 161,78 Q144,72 128,86 Z" fill="#f9a8d4"/><g transform="translate(206,30) rotate(14)"><path d="M0,28 4,6 13,18 22,2 31,18 40,6 44,28 Z" fill="url(#horn)"/><path d="M0,28 h44 v8 a4,4 0 0 1 -4,4 H4 a4,4 0 0 1 -4,-4 Z" fill="#f2b64b"/><circle cx="10" cy="34" r="3.5" fill="#f44fa0"/><circle cx="22" cy="34" r="3.5" fill="#f44fa0"/><circle cx="34" cy="34" r="3.5" fill="#f44fa0"/></g><ellipse cx="108" cy="164" rx="10" ry="6" fill="#ffc3dd" opacity=".85"/><ellipse cx="212" cy="164" rx="10" ry="6" fill="#ffc3dd" opacity=".85"/><circle cx="130" cy="140" r="11" fill="#3b2340"/><circle cx="126" cy="136" r="4" fill="#fff"/><circle cx="134" cy="145" r="2" fill="#fff"/><circle cx="190" cy="140" r="11" fill="#3b2340"/><circle cx="186" cy="136" r="4" fill="#fff"/><circle cx="194" cy="145" r="2" fill="#fff"/><ellipse cx="149" cy="178" rx="3.5" ry="5" fill="#d98aa8"/><ellipse cx="171" cy="178" rx="3.5" ry="5" fill="#d98aa8"/><path d="M151,192 Q160,200 169,192" stroke="#b5688c" stroke-width="4.5" fill="none" stroke-linecap="round"/><path d="M0,-13 Q2.5,-2.5 13,0 Q2.5,2.5 0,13 Q-2.5,2.5 -13,0 Q-2.5,-2.5 0,-13 Z" transform="translate(54,54)" fill="#fcd34d"/><path d="M0,-13 Q2.5,-2.5 13,0 Q2.5,2.5 0,13 Q-2.5,2.5 -13,0 Q-2.5,-2.5 0,-13 Z" transform="translate(268,70) scale(.8)" fill="#f472b6"/><path d="M0,-13 Q2.5,-2.5 13,0 Q2.5,2.5 0,13 Q-2.5,2.5 -13,0 Q-2.5,-2.5 0,-13 Z" transform="translate(70,196) scale(.75)" fill="#c4b5fd"/><path d="M0,-13 Q2.5,-2.5 13,0 Q2.5,2.5 0,13 Q-2.5,2.5 -13,0 Q-2.5,-2.5 0,-13 Z" transform="translate(246,222) scale(.9)" fill="#fcd34d"/></svg>`;
const SETS=[
...
function feedback(text,cls){const el=$('.feedback');if(!el)return;el.textContent=text;el.className='feedback '+cls}
...
function renderBlock(){const q=current();
  app.innerHTML=headerBar('Letter Blocks',`<span class="pill">${state.i+1} / ${state.queue.length}</span>`)+`<section class="bstage"><div class="card">${visual(q.word)}<button class="sound" aria-label="Replay word">🔊</button>${state.showWord?`<div class="reveal">${q.word.toUpperCase()}</div>`:'<p class="prompt">Build the word</p>'}<div class="feedback"></div></div>${blocksBoard()}</section>`;
...
function startBlocks(idx){const words=SETS[idx].map(word=>({word,set:idx+1}));
  state={screen:'blocks',mode:'blocks',setIndex:idx,words,queue:words,i:0,round:1,leg:'cards',attempts:0,hinted:false,locked:false,showWord:true,accent:'us',answers:[],tiles:[],slots:[],fixed:new Set()};
  cardLeg()}
function cardLeg(){const q=current(),last=state.i===state.queue.length-1;state.accent=accentFor(q.word);
  app.innerHTML=headerBar('Word Cards',`<span class="pill">${state.i+1} / ${state.queue.length}</span>`)+`<section class="bstage"><div class="card">${visual(q.word)}<button class="sound" aria-label="Replay word">🔊</button><div class="reveal">${q.word.toUpperCase()}</div><p class="prompt">Look at it. Listen to it. Say it.</p><div class="actions"><button class="primary go">${last?'Build the words →':'Next →'}</button></div></div></section>`;
...
function blocksPanel(inner){app.innerHTML=headerBar('Letter Blocks')+`<section class="bstage"><div class="card">${inner}</div></section>`;wireBack(()=>chooseSets('blocks'));window.onkeydown=null}
function roundBreak(){blocksPanel(`<div class="reveal">Round 1 done ⭐</div><p class="prompt">The same five words again, all mixed up.</p><div class="actions"><button class="primary go">Go →</button></div>`);$('.go').onclick=beginRound}
