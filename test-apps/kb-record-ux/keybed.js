 ...
 *
 * Two layouts sit on the same machinery:
 ...
 *
 * A third, `chunky` — the First Piano's old two-row bed — was retired when
 * the First Piano took the Princess arrangement (owner, 2026-09-14; see
 * TOY-SOUND-EXPANSION-SPEC.md §1): it had no user left, and a dead layout
 * that nothing documents as dead is a trap for the next reader.
 *
...
(function () {
  ...
  KB.mountKeybed = function mountKeybed(el, opts) {
    ...
    /* `front` is the row along the bottom — the one a palm lands on. On a
       piano bed that is the naturals; the accidentals are drawn over their
       boundaries instead of in a row of their own. */
    const front = flat ? notes : notes.filter((m) => !IS_BLACK.has(m % 12));
    const whites = front;
    ...
    function build() {
      ...
      whites.forEach((m, i) => {
        ...
        d.className = flat ? "pad" : ("white" + (m % 12 === 0 ? (m === 60 ? " c c4" : " c") : ""));
    ...
    function keyAt(x, y) {
      ...
      /* The flat bed's gaps play the neighbour, on purpose. Owner, 2026-09-13:
         the gaps between the big keys "play neighbor" — a palm landing between
         two slabs sounds one of them rather than nothing. That overrides
         TOY-ARCHITECTURE.md's "null outside every playable surface" for the
         young toys' front rows. Do not "fix" it back to silent. (The First
         Piano's old chunky front row carried the same ruling; that row is
         gone, and its replacement bed is an ordinary piano bed.) */
