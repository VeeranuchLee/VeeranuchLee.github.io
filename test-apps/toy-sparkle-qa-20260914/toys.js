...
(function () {
  ...
  /* ----------------------------------------------------- 🧸 Preschool ---
     Ten broad note slabs along the bottom and five chunky accidental buttons
     in a separate rear row: fifteen keys, the two rows clearly apart, which
     is what TOY-ARCHITECTURE asks for.

     The note set is C4-E5 rather than the document's C4-D5, and that is a
     deliberate correction to one line of it. C4-D5 chromatic is fifteen
     notes of which SIX are accidentals, so "fifteen keys ... with five short
     raised accidental buttons" cannot both be true: the earlier build made
     the numbers add up by drawing C#5 as a broad white-note slab, which puts
     a semitone in the middle of a row a child slides a palm across. Ten
     naturals C4-E5 plus the five accidentals of the C octave is fifteen keys,
     five rear buttons, and no accidental hiding in the front row. */

  const PRESCHOOL_FRONT = [60, 62, 64, 65, 67, 69, 71, 72, 74, 76]; /* C4-E5 naturals */
  const PRESCHOOL_REAR = new Set([61, 63, 66, 68, 70]);             /* C#4-A#4 */
  const PRESCHOOL_NOTES = PRESCHOOL_FRONT.concat([...PRESCHOOL_REAR]).sort((a, b) => a - b);
    ...
    mount(host) {
      const root = frag(
        '<div class="toy toy-preschool">' +
          ...
          '</div>' +
          '<section class="keybed chunky" aria-label="Keys"></section>' +
        '</div>');
      host.appendChild(root);
      KB.engine.setSustain(false);
      ...
      setVoice(voice);
      /* Between the sound buttons and the slabs, styled in toys.css as one
         more of the case's own chunky buttons. */
      const unsubscribeRecorder = mountTransport(root, root.querySelector(".keybed"));
      const effects = KB.mountPressEffects(root, "preschool");
      const bed = KB.mountKeybed(root.querySelector(".keybed"), {
        layout: "chunky",
        notes: PRESCHOOL_NOTES,
        rear: [...PRESCHOOL_REAR],
        computerKeyboard: false,
        decorate(d, midi) {
          d.classList.add(PRESCHOOL_REAR.has(midi) ? "rear-accidental" : "broad-slab");
          /* Ten colours in the order a child meets them, so a slab is a
             colour before it is a note. The hit test never reads this. */
          if (!PRESCHOOL_REAR.has(midi)) d.classList.add("hue" + PRESCHOOL_FRONT.indexOf(midi));
          if (!PRESCHOOL_REAR.has(midi)) {
            d.dataset.effectColour = ["#ff8d99", "#ffa86f", "#ffe17a", "#d7eb74", "#79dfb1", "#75dce5", "#8bb2ff", "#bca0ff", "#ed9bd8", "#ffadc7"][PRESCHOOL_FRONT.indexOf(midi)];
          }
