...
function applyAtmosphere() {
  ...
  if (currentView === 'landing') {
    ...
    stopBed();
    if (hasScene(journey.roomId)) ambience.start(journey.roomId);
    else ambience.stop();
  } else {
    stopBed();
    ambience.stop();
...
function playPiece(id, which) {
  ...
  player.onFinish = () => { engine.duck('piece', false); setPlayingUI(null, null); };
...
sound.load();
syncSoundButton();
window.musicBook = { engine, player, bed, ambience, sound, journey };
