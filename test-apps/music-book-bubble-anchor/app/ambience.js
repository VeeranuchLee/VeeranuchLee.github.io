// Environmental sound, built live from oscillators and filtered noise.
//
// PORTABLE ON PURPOSE — this module names no app, no room and no page. It takes
// a list of layer names and builds them on the engine's `ambientBed` bus; it
// holds no map from any book's pages to any scene. That map belongs to the book.
//
// **The Music Book does not use this.** AUDIO-DIRECTION.md decision 12 made that
// book music-first: its background is a tune played by the child's companion,
// not a place. The class is still constructed and wired there so the capability
// stays exercised rather than rotting, and its scene map is empty, so it is
// present, wired and silent.
//
// It is kept because environmental sound carries meaning in books the Music Book
// is not — the Storybook, the Animal Book, habitat and nature scenes: forest ->
// birds and leaves, ocean -> waves, rainforest -> rain and frogs, night ->
// crickets. To use it there, hand `start()` a scene id and the layers that scene
// wants:
//
//     ambience.start('rainforest-canopy', ['water', 'birds']);
//
// The owner's rule travels with it: do not force ambience into every scene. It
// earns its place where it represents the world of the page, and nowhere else.
//
// Charming and readable beats realistic: the goal is a picture-book cue that
// stays well under everything else, not a field recording.
...
// The layers this module can build. A book's scene map may only name these.
export const LAYERS = ['breeze', 'hall', 'water', 'birds'];
...
export class Ambience {
   ...
   * Start a scene.
   *
   * @param {string} sceneId  the caller's own id for this scene — only used to
   *                          notice that the same scene is already running
   * @param {string[]} layers  names from `LAYERS`. An empty or missing list
   *                           stops the bed, which is how a book that uses
   *                           ambience on some pages and not others says so.
   */
  start(sceneId, layers) {
    if (this.sceneId === sceneId && this.sources.length) return;
    this.stop();
    if (!layers?.length || !this.engine.ctx) return;
    this.sceneId = sceneId;
    layers.forEach((layer) => {
      if (!LAYERS.includes(layer)) throw new Error(`no ambience layer named: ${layer}`);
      this[`_${layer}`]();
    });
  }
  ...
  _noiseLayer({ type, frequency, Q, level, sway, swayRate, send = 0 }) {
    const ctx = this.engine.ctx;
    const bus = this.engine.bus('ambientBed');
    if (!ctx || !bus) return;
